...
export function reportRoutes(db) {
   ...
   * ========================================================== */
  const monthlyData = async (from, to, wh = { sql: '', params: {} }) => {
  ...
  router.get('/monthly', wrap(async (req, res) => {
    ...
    const { inbound, outbound } = await monthlyData(from, to, whFilter(req));
    res.json({
      from, to, year, month,
      inbound,
      outbound,
  ...
  router.get('/monthly.csv', wrap(async (req, res) => {
    ...
    const { outbound } = await monthlyData(from, to, whFilter(req));
  ...
  /* ---------- สรุปการเบิกตามแผนก ---------- */
  router.get('/by-department', wrap(async (req, res) => {
    const { from, to } = rangeFrom(req);
  ...
  /* ---------- สรุปตามหมวดหมู่ ---------- */
  router.get('/by-category', wrap(async (req, res) => {
    const { from, to } = rangeFrom(req);
    const data = await db.all(`
  ...
  router.get('/top-items', wrap(async (req, res) => {
    const { from, to } = rangeFrom(req);
  ...
  router.get('/assets-by-holder.csv', wrap(async (req, res) => {
  ...
  router.get('/charge-summary', wrap(async (req, res) => {
    const { from, to } = rangeFrom(req);
    const wh = whFilter(req);
    const data = await db.all(`
      SELECT COALESCE(dp.name, '(ไม่ระบุแผนก)') AS department_name,
  ...
  router.get('/audit', wrap(async (req, res) => {
    ...
    const total = (await db.get('SELECT COUNT(*) AS n FROM audit_logs')).n;
    const data = await db.all('SELECT * FROM audit_logs ORDER BY id DESC LIMIT @limit OFFSET @offset', { limit, offset: (page - 1) * limit });
...
export function dashboardRoutes(db) {
  ...
  router.get('/', wrap(async (req, res) => {
    ...
    const recentMoves = await db.all(`
