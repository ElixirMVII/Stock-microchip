...
import { lookup, can, refreshLowBadge } from '../app.js';
import {
...
export async function renderReceipts(view) {
  ...
  const load = async () => {
    ...
    const res = await api.get(`/receipts${api.qs(q)}`);
...
export async function renderIssues(view) {
  ...
  const load = async () => {
    const box = view.querySelector('#list');
    box.innerHTML = loading();
    const res = await api.get(`/issues${api.qs(q)}`);
    box.innerHTML = table(res.data, [
      { key: 'doc_no', label: 'เลขที่เอกสาร', render: (r) => `<b class="mono">${esc(r.doc_no)}</b>` },
...
export async function renderReturns(view) {
  ...
  const load = async () => {
    const box = view.querySelector('#list');
    box.innerHTML = loading();
    const res = await api.get(`/returns${api.qs(q)}`);
    box.innerHTML = table(res.data, [
      { key: 'doc_no', label: 'เลขที่เอกสาร', render: (r) => `<b class="mono">${esc(r.doc_no)}</b>` },
...
async function returnForm(reload) {
  ...
  modal({
    ...
    onMount: (root) => {
      root._lines = editor.mount(root, { employeeSelect: root.querySelector('#ret-emp') });
    },
    onSubmit: async (data, { close, root }) => {
      const payload = { ...data, lines: root._lines.collect() };
...
export async function renderAdjustments(view) {
  ...
  const load = async () => {
    const box = view.querySelector('#list');
    box.innerHTML = loading();
    const res = await api.get(`/adjustments${api.qs(q)}`);
    box.innerHTML = table(res.data, [
      { key: 'doc_no', label: 'เลขที่เอกสาร', render: (r) => `<b class="mono">${esc(r.doc_no)}</b>` },
...
async function adjustmentForm(reload) {
  ...
  const balances = (await api.get('/stock/balance?per_page=1000')).data;
  const balOf = new Map(balances.map((b) => [b.item_id, b.balance]));
  let n = 0;
